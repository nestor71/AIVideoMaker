...
class VideoTranslator:
    ...
    def translate_video(
        ...
        try:
            ...
            if enable_lipsync:
                self._update_progress(progress_callback, 10, "🎙️ Uso ElevenLabs per dubbing professionale con lip-sync...")
                ...
                try:
                    ...
                    if success:
                        self._update_progress(progress_callback, 100, "✅ Dubbing con lip-sync completato!")
                        return True
                    else:
            ...
            self._update_progress(progress_callback, 100, "✅ Traduzione completata!")
            ...
            self._update_progress(progress_callback, -1, f"❌ Errore: {str(e)}")
            return False
    ...
    def _transcribe_audio(self, audio_path: str, source_language: str = 'auto') -> dict:
        ...
        # Carica modello Whisper (usa "base" per velocità, "medium/large" per accuratezza)
    ...
    def _translate_text(self, text: str, target_language: str) -> str:
        ...
        try:
            ...
            # Traduci in chunks per evitare limiti API (deep-translator ha limite 5000 caratteri)
            max_chunk_size = 4900  # Lascia margine di sicurezza
            ...
            raise Exception(
                "deep-translator non installato. Esegui: pip install deep-translator"
            )
        except Exception as e:
    ...
    def _generate_tts(self, text: str, language: str) -> str:
        ...
        # Converti MP3 -> WAV per compatibilità
    ...
    def _apply_lipsync(self, video_path: str, audio_path: str) -> str:
        ...
        ⚠️ ATTENZIONE: Questa funzione è MOLTO LENTA su CPU (10-30 minuti per video brevi)
        ...
        """
        # NOTA: Questa è una implementazione placeholder
        # Wav2Lip richiede setup complesso con modelli pre-trained, GPU, etc.

        logger.warning(
            "⚠️ Lip-sync richiede Wav2Lip installato e configurato. "
    ...
    def _combine_video_audio(
        ...
        Args:
            video_path: Path video (può avere lip-sync già applicato)
            audio_path: Path audio tradotto
        ...
        # Se audio è più corto del video, estende con silenzio
        # Se audio è più lungo, taglia il video alla durata dell'audio
        ...
        cmd = [
            ...
            '-shortest',  # Taglia alla durata più corta
            '-y',  # Overwrite
            output_path
        ]

...
def cleanup_old_files():
    """Rimuove file più vecchi di FILE_RETENTION_DAYS"""
    try:
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("🚀 Avvio AI Video Maker")
    load_jobs_state()
    cleanup_old_files()
    yield
    # Shutdown
    logger.info("💾 Salvataggio stato jobs...")
    save_jobs_state()
...
async def home(request: Request):
    response = templates.TemplateResponse("index_new.html", {"request": request})
...
async def workflow_automatico(
    ...
    """
    # Verifica che almeno una funzionalità sia attiva
    if not any([logo_overlay, call_to_action, video_transcription, audio_translation,
        ...
        raise HTTPException(
            status_code=400,
            detail="Seleziona almeno una funzionalità da eseguire"
        )
    ...
    try:
        ...
        if audio_translation and target_language:
            ...
            try:
                ...
                # Questa è una semplificazione - la traduzione vera richiede più parametri
                # Per ora logghiamo che la feature è attiva
        ...
        if youtube_upload and metadata_result:
            ...
            try:
                ...
                # Questa è una semplificazione - l'upload richiede autenticazione OAuth
...
async def delete_job(job_id: str):
    ...
    if job.status == "completed":
        logger.info(f"Job {job_id} già completato - mantengo output file")
        with jobs_lock:
            del processing_jobs[job_id]
        save_jobs_state()
        return {"message": "Job rimosso (era già completato)"}
...
def get_file_info(file_path: Path, directory: str):
    ...
    # Video files
    if suffix in ['.mp4', '.avi', '.mov', '.mkv', '.webm', '.flv', '.wmv', '.m4v', '.3gp', '.ogv', '.ts', '.mpg', '.mpeg']:
        file_info["type"] = "video"
        file_info["category"] = "Video"
        file_info["metadata"] = get_video_metadata(file_path)
    # Image files
    elif suffix in ['.png', '.jpg', '.jpeg', '.gif', '.svg', '.webp', '.bmp', '.tiff', '.tif', '.ico']:
        file_info["type"] = "image"
        file_info["category"] = "Immagine"
        file_info["metadata"] = get_image_metadata(file_path)
    # Subtitle files
    elif suffix in ['.srt', '.vtt', '.ass', '.ssa', '.sub']:
        file_info["type"] = "subtitle"
        file_info["category"] = "Sottotitoli"
        file_info["metadata"] = get_subtitle_metadata(file_path)
    # Audio files
    elif suffix in ['.mp3', '.wav', '.aac', '.ogg', '.flac', '.m4a', '.wma', '.opus']:
        file_info["type"] = "audio"
        file_info["category"] = "Audio"
        file_info["metadata"] = get_audio_metadata(file_path)
    # Document files
    elif suffix in ['.txt', '.log', '.pdf', '.doc', '.docx', '.rtf', '.odt']:
        file_info["type"] = "document"
        file_info["category"] = "Documento"
        file_info["metadata"] = {"lines": count_file_lines(file_path)} if suffix in ['.txt', '.log'] else {}
    # Archive files
    elif suffix in ['.zip', '.rar', '.7z', '.tar', '.gz', '.bz2', '.xz']:
        file_info["type"] = "archive"
        file_info["category"] = "Archivio"
        file_info["metadata"] = {}
    # Data files
    elif suffix in ['.json', '.xml', '.csv', '.yaml', '.yml', '.toml']:
        file_info["type"] = "data"
        file_info["category"] = "Dati"
        file_info["metadata"] = {"lines": count_file_lines(file_path)} if suffix != '.json' else {}
    else:
...
async def stop_screen_recording(recording_id: str):
    ...
    try:
        ...
        # Ferma recording (può richiedere qualche secondo)
...
async def download_web_video(url: str = Form(...)):
    ...
    try:
        ...
        # Controlla se è un link diretto a file video
...
async def transcribe_video(file_id: str = Form(...)):
    ...
    try:
        ...
        # Carica il modello Whisper (usa "base" per velocità, "medium" o "large" per accuratezza)
        model = whisper.load_model("base")
...
def process_thumbnail_generation(
    ...
    try:
        ...
        # Salva con alta qualità ma sotto i 2MB (limite YouTube)
        quality = 95
        base_image.save(output_path, 'JPEG', quality=quality, optimize=True)

        # Se supera 2MB, riduci qualità
        while output_path.stat().st_size > 2 * 1024 * 1024 and quality > 60:
            quality -= 5
            base_image.save(output_path, 'JPEG', quality=quality, optimize=True)

        file_size_mb = output_path.stat().st_size / (1024 * 1024)
        logger.info(f"Miniatura salvata: {output_path} ({file_size_mb:.2f} MB, qualità {quality})")
...
def add_text_to_thumbnail(
    ...
    try:
        ...
        # Disegna sfondo del testo con opacità
...
def enhance_thumbnail_for_ctr(image: Image.Image) -> Image.Image:
    ...
    try:
        ...
        # Aumenta leggermente saturazione per colori più vivaci
    ...
    try:
        ...
        if success and output_path.exists():
            job.update(
                status="completed",
                progress=100,
                message="✅ Video tradotto con successo!",
...
async def youtube_status():
    """Verifica se l'utente è autenticato con YouTube"""
    try:
                ...
                <html>
                    ...
                    <body>
                        <div class="container">
                            ...
                            <p>Il tuo account YouTube è stato collegato con successo.</p>
...
async def youtube_upload_video(
    ...
    Args:
        ...
        privacy: Visibilità (private, unlisted, public)
    """
    try:
    ...
    try:
        ...
        if result.get('success'):
            ...
            job.update(
                status="completed",
                progress=100,
                message=f"✅ Video caricato su YouTube!",
        ...
        job.update(
            status="error",
            message=f"❌ Errore upload YouTube: {str(e)}",
            error=str(e)
        )
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            for file_path in request.files:
                # Converti path relativo (/uploads/file.webm) in path assoluto
                if file_path.startswith("/uploads/"):
                    full_path = UPLOAD_DIR / file_path.replace("/uploads/", "")
                elif file_path.startswith("/outputs/"):
                    full_path = OUTPUT_DIR / file_path.replace("/outputs/", "")
                ...
                if not full_path.exists():
                    logger.warning(f"File non trovato: {file_path} -> {full_path}")
                    continue
